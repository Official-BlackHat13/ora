...
class JobController extends Controller
{
    ...
    public function Apply(Request $request)
    {
        ...
        if($request->graduation_cer){
            //  use for upload thenth certificate
            $tenth=$this->fileUpload($request->graduation_cer,'accdmic_N_Professional',$request->job_id);
            $request->request->remove('graduation_cer');
            $request->request->add(['graduation_cer_url'=>$tenth]);
        }
        ...
        $app->user_data=json_encode($request->all(),true);
        $app->save();
        if($app){
